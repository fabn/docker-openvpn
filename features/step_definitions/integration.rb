Given(/^a configured OpenVPN instance with "([^"]*)"$/) do |flags|
  ...
  run_simple %Q{docker run --volumes-from #{@data_volume} --rm kylemanna/openvpn ovpn_genconfig -u udp://ovpn-server #{flags}}
...
Given(/^a running OpenVPN process$/) do
  ...
  run %Q{docker run --volumes-from #{@data_volume} --name ovpn-server --rm -p 1194:1194 --privileged kylemanna/openvpn}, startup_wait_time: 5
...
When(/^I run "([^"]*)" openvpn client in docker$/) do |client_name|
  ...
  cd 'clients' do
    ...
    run %Q{docker run #{docker_options} kylemanna/openvpn #{openvpn_client}}, startup_wait_time: 5
    # Wait for connection established in VPN
    step %q{I wait for output to contain "Initialization Sequence Completed"}
  end
